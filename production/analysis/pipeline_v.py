...
def get_data_extracter(
    ...
    if not config.data_reader.is_valid(db_path):
        ...
        data_builder._align_datasets(plot_dir, config.hydraulic_parameter)
        if save:
...
def gen_plots(
    ...
    performance_scatterplot(
        ...
        hydraulic_parameters=config.hydraulic_parameter
    )
    performance_cdf(
        lf_test_data_df.values,
        hf_test_data_df.values,
    ...
    map_mesh_errors(
        ...
        error_metric="Max Velocity Error" if hydraulic_parameters == "velocity" else "Max Depth Error",
    )
...
def pipeline(config: Config) -> None:
    ...
    if config.hydraulic_parameter == "velocity":
        lf_test_data_val = lf_test_data
        hf_test_data_val = hf_test_data
        y_test_pred_val = y_test_pred
    else:
        lf_test_data_val = reducer.wse_2_depth(lf_test_data) # lf_test_data_depth
        hf_test_data_val = reducer.wse_2_depth(hf_test_data) # hf_test_data_depth
        y_test_pred_val = reducer.wse_2_depth(y_test_pred) # y_test_pred_depth

    ### Assess performance and plot diagnostics ###
    t5 = time.perf_counter()
    print("Calculating metrics and making performance plots")
    export_metric_summary(
        pd.DataFrame(hf_test_data_val, index=hf_test_data_df.index, columns=hf_test_data_df.columns),
        pd.DataFrame(y_test_pred_val, index=hf_test_data_df.index, columns=hf_test_data_df.columns),
        config.metric_db_path,
        hydraulic_parameter=config.hydraulic_parameter
    )
    ...
    if config.generate_plots:
        gen_plots(
            ...
            y_test_pred_val,
            reducer.eofs,
            extracter.hf_geometry_aoi[config.cell_id_field][~reducer.dry_indices].tolist() if config.hydraulic_parameter != "velocity" else extracter.hf_geometry_aoi[config.cell_id_field].tolist(),
            config.hydraulic_parameter
        )
...
if __name__ == "__main__":
    config = Config.from_file("data/velocity_upskill/pipeline.config.json")
    testing = True
