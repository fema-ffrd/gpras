...
def pipeline(config: Config) -> None:
    ...
    ### Load data needed to mask fluvial data ***
    print("Loading fluvial mask")
    path_fluvial_surface = r"/workspaces/gpras/production/post_processing/GIS/masks/gpr40_fluvial_surface_t55.shp"
    gdf_fluvial_surface = gpd.read_file(path_fluvial_surface).sort_values(by="cell_id", ascending=True)
    gdf_fluvial_surface["is_fluvial"] = (
        gdf_fluvial_surface.p_plu_flu > 0
    )  # use 0 ft as threshold definition for fluvial
    # list_fluvial_cell_ids = gdf_fluvial_surface.loc[gdf_fluvial_surface.is_fluvial == True, "cell_id"].to_list()

    ### Mask data to only include fluvial data ###
    print("Masking non-fluvial data")
    # variabels that don't need to be masked: x, y, x_test, y_text, mean_pred
    # this statement is "True", proving cell_ids in columns are ascending:
    # list(sorted(lf_test_data_df.columns.values)) == list(lf_test_data_df.columns.values)
    hf_data_df = hf_data_df.loc[:, gdf_fluvial_surface.is_fluvial.values]  # assumes cell_id columns are ascending
    lf_test_data_df = lf_test_data_df.loc[
        :, gdf_fluvial_surface.is_fluvial.values
    ]  # assumes cell_id columns are ascending
    hf_test_data_df = hf_test_data_df.loc[
        :, gdf_fluvial_surface.is_fluvial.values
    ]  # assumes cell_id columns are ascending
    y_test_pred = y_test_pred[:, gdf_fluvial_surface.is_fluvial]  # assumes columns representing ascending cell_ids
    lf_test_data_depth = lf_test_data_depth[:, gdf_fluvial_surface.is_fluvial.values]
    hf_test_data_depth = hf_test_data_depth[:, gdf_fluvial_surface.is_fluvial.values]
    y_test_pred_depth = y_test_pred_depth[:, gdf_fluvial_surface.is_fluvial.values]
    ...
    export_metric_summary(
        ...
        "data/ras_upskill_fluvial/metrics/performance_metrics_lf.db",
        v_tol=1.0,
        t_tol=1,  # added by DCW
    )
...
if __name__ == "__main__":
    config = Config.from_file("data/ras_upskill_fluvial/pipeline_fluvial.config.json")
    pipeline(config)
