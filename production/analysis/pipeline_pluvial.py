...
def pipeline(config: Config) -> None:
    ...
    ### Load data needed to mask pluvial data ***
    print("Loading pluvial mask")
    ...
    gdf_fluvial_surface["is_pluvial"] = (
    ...
    hf_data_df = hf_data_df.loc[:, gdf_fluvial_surface.is_pluvial.values]  # assumes cell_id columns are ascending
    lf_test_data_df = lf_test_data_df.loc[
        :, gdf_fluvial_surface.is_pluvial.values
    ]  # assumes cell_id columns are ascending
    hf_test_data_df = hf_test_data_df.loc[
        :, gdf_fluvial_surface.is_pluvial.values
    ]  # assumes cell_id columns are ascending
    y_test_pred = y_test_pred[:, gdf_fluvial_surface.is_pluvial]  # assumes columns representing ascending cell_ids
    lf_test_data_depth = lf_test_data_depth[:, gdf_fluvial_surface.is_pluvial.values]
    hf_test_data_depth = hf_test_data_depth[:, gdf_fluvial_surface.is_pluvial.values]
    y_test_pred_depth = y_test_pred_depth[:, gdf_fluvial_surface.is_pluvial.values]
...
if __name__ == "__main__":
    config = Config.from_file("data/ras_upskill_pluvial/pipeline_pluvial.config.json")
    pipeline(config)
